...
/**
 * Single-slit Fraunhofer intensity at screen position y (m).
 * I = A² · sinc²(πa sinθ / λ),  sinθ = y/√(y²+L²)
 */
function intensity(y) {
  const { lambda, a, L, amplitude } = state;
  const sinTheta = y / Math.sqrt(y*y + L*L);
  const beta = a * sinTheta / lambda;
  const I = amplitude * amplitude * sinc(beta) * sinc(beta);
...
function drawWaveField() {
  ...
  // Sample N point sources across the slit aperture for Huygens construction
  const N_SOURCES = 7;
  const sources = [];
  for (let i = 0; i < N_SOURCES; i++) {
    const frac = N_SOURCES === 1 ? 0 : (i / (N_SOURCES - 1) - 0.5);
    sources.push(cy + frac * slitHalfPx * 2);
  }
  ...
  for (let py = 0; py < H; py++) {
    for (let px = 0; px < W; px++) {
      ...
      // Left of barrier: plane wave stripes
      if (px < cx) {
      ...
      // Sum contributions from all Huygens sources across slit
      let wSum = 0;
      let ampSum = 0;
      ...
      // Combined constructive glow
      const normC = wSum / (ampSum + 1e-9);
      const glow  = Math.pow(Math.max(0, normC), 2.5) * 0.35;
  ...
  wCtx.fillRect(bx, 0,                         barrierW, cy - slitHalfPx);
  wCtx.fillRect(bx, cy + slitHalfPx,            barrierW, H - (cy + slitHalfPx));
  wCtx.strokeStyle = '#4a6fa5';
  wCtx.lineWidth = 1.5;
  [[0, cy - slitHalfPx], [cy + slitHalfPx, H - (cy + slitHalfPx)]].forEach(([y, h]) => {
    if (h > 0) wCtx.strokeRect(bx, y, barrierW, h);
  });
  // Slit gap highlight
  wCtx.strokeStyle = '#ffffff44';
  wCtx.lineWidth = 1;
  wCtx.beginPath();
  wCtx.moveTo(bx, cy - slitHalfPx); wCtx.lineTo(bx + barrierW, cy - slitHalfPx);
  wCtx.moveTo(bx, cy + slitHalfPx); wCtx.lineTo(bx + barrierW, cy + slitHalfPx);
  wCtx.stroke();

  // Slit halo
  const grad = wCtx.createRadialGradient(cx, cy, 0, cx, cy, slitHalfPx * 5 + 12);
  grad.addColorStop(0,   `rgba(${SR},${SG},${SB},0.9)`);
  grad.addColorStop(0.3, `rgba(${SR},${SG},${SB},0.4)`);
  grad.addColorStop(1,   `rgba(${SR},${SG},${SB},0)`);
  wCtx.save();
  wCtx.beginPath();
  wCtx.rect(cx - 4, cy - slitHalfPx * 5 - 12, 28, slitHalfPx * 10 + 24);
  wCtx.clip();
  wCtx.fillStyle = grad;
  wCtx.fillRect(cx - 4, cy - slitHalfPx * 5 - 12, 28, slitHalfPx * 10 + 24);
  wCtx.restore();

  // Barrier and source labels
  wCtx.save();
  ...
  wCtx.save(); wCtx.font = 'bold 8px monospace'; wCtx.fillStyle = '#58a6ffcc';
  wCtx.textAlign = 'center'; wCtx.fillText('SOURCE', gunX + gunW / 2 + 2, gunY - 6);
  ...
  for (let sy = 0; sy < H; sy++) {
    ...
    for (let sx = 0; sx < SCREEN_W; sx++) {
      ...
      sd[si]   = Math.round(wr * I); sd[si+1] = Math.round(wg * I);
      sd[si+2] = Math.round(wb * I); sd[si+3] = 255;
    }
  }
  wCtx.putImageData(screenImg, screenStartX, 0);
  wCtx.strokeStyle = '#58a6ff'; wCtx.lineWidth = 1.5;
  wCtx.strokeRect(screenStartX, 0, SCREEN_W, H);

  // Screen label
  wCtx.save(); wCtx.font = 'bold 8px monospace'; wCtx.fillStyle = '#58a6ffcc';
  wCtx.textAlign = 'center';
  wCtx.translate(screenStartX + SCREEN_W / 2, H / 2);
  wCtx.rotate(-Math.PI / 2); wCtx.fillText('DETECTOR SCREEN', 0, 0); wCtx.restore();

  // Distance annotation
  const annotY = H - 14;
  wCtx.save(); wCtx.strokeStyle = 'rgba(255,255,255,0.22)'; wCtx.lineWidth = 1;
...
function drawIntensityGraph() {
  ...
  // Range: ±5 first-order minima
  const y1 = state.lambda * state.L / state.a;
  const yRange = Math.max(y1 * 5, 1e-4);
  ...
  for (let m = -4; m <= 4; m++) {
    if (m === 0) continue;
    const y_m = m * state.lambda * state.L / state.a;
    const py  = PAD_T + plotH / 2 - (y_m / yRange) * (plotH / 2);
    if (py >= PAD_T && py <= PAD_T + plotH) {
      gCtx.strokeStyle = '#30363d66'; gCtx.beginPath();
      gCtx.moveTo(PAD_L - 3, py); gCtx.lineTo(PAD_L, py); gCtx.stroke();
    }
  }
  ...
  if (state.showLabels) {
    for (let m = -4; m <= 4; m++) {
      ...
      if (px_m >= PAD_L && px_m <= PAD_L + plotW) {
        gCtx.strokeStyle = '#f0a04055'; gCtx.setLineDash([2,3]);
        gCtx.beginPath(); gCtx.moveTo(px_m, PAD_T); gCtx.lineTo(px_m, PAD_T + plotH); gCtx.stroke();
        gCtx.setLineDash([]);
        gCtx.fillStyle = '#f0a04099'; gCtx.textAlign = 'center'; gCtx.font = '9px monospace';
...
function updateFormulaPanel() {
  const { lambda, a, L, amplitude } = state;
  const y1 = lambda * L / a;
  const theta1_deg = (Math.asin(Math.min(1, lambda / a)) * 180 / Math.PI).toFixed(2);

  document.getElementById('central-width-val').textContent =
    `y₁ = ${(y1 * 1000).toFixed(3)} mm`;
  document.getElementById('angular-width-val').textContent =
    `2θ = ${(2 * parseFloat(theta1_deg)).toFixed(2)}°`;
  ...
  document.getElementById('p-A').textContent      = `${amplitude.toFixed(1)}`;

  const table = document.getElementById('minima-table');
  table.innerHTML = '';
  for (let m = -4; m <= 4; m++) {
    if (m === 0) continue;
    const y_m = m * lambda * L / a;
